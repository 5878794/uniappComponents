...
let all = {
	...
	openPage(url,closeType){
		...
		}else if(closeType == 'onlyOne'){
			//跳转到首页后再打开新页面
			let _this = this;
			this.goBack(-1,function(){
				_this.openPage(url);
			});
	...
	imgSrcRelToAbs(data,addImgServerUrl){
		...
		let getFileExt = function(str){
			//判断是否是带空格的字符串
			//带空格的字符串肯定不是普通的文本字符串 可能是富文本  不处理
	...
	//打开内置地图
	//微信可以打开  其他端没测试
	openMapPage(lat,lng,name,address){
		wx.openLocation({
